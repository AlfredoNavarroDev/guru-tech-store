...
export class AddSedeToPromociones1784000000004 implements MigrationInterface {
  name = 'AddSedeToPromociones1784000000004';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Add id_sede column to promociones (NULL = applies to all sedes)
    await queryRunner.query(`
      ALTER TABLE promociones
        ADD COLUMN IF NOT EXISTS id_sede INT NULL
          REFERENCES sedes(id_sede)
    `);

    // 2. Drop and recreate v_vendedor_catalogo with sede-aware promo filtering
    ...
    await queryRunner.query(`
      ...
      LEFT JOIN promo_vigente pv   ON pv.id_item = i.id_item
                                   AND (pv.promo_id_sede IS NULL OR pv.promo_id_sede = inv.id_sede)
      WHERE i.tipo = 'producto'
    `);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // 1. Restore v_vendedor_catalogo without sede filtering
    await queryRunner.query(`DROP VIEW IF EXISTS v_vendedor_catalogo`);
    await queryRunner.query(`
      CREATE VIEW v_vendedor_catalogo AS
      WITH promo_vigente AS (
    ...
    // 2. Drop id_sede column from promociones
    await queryRunner.query(`
      ALTER TABLE promociones DROP COLUMN IF EXISTS id_sede
    `);
  }
}
