...
const BentoCard = ({ name, className, background, Icon, description, href, cta, ...props }: BentoCardProps) => (
  <div
    ...
    className={cn(
      ...
      "bg-zinc-900 [box-shadow:0_0_0_1px_rgba(255,255,255,.05),0_2px_4px_rgba(0,0,0,.3)]",
      "dark:bg-zinc-900 dark:[border:1px_solid_rgba(255,255,255,.08)]",
      className
    )}
    {...props}
  >
    ...
    <div className="p-4">
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 transition-all duration-300 lg:group-hover:-translate-y-10">
        <Icon className="h-10 w-10 origin-left transform-gpu text-neutral-400 transition-all duration-300 ease-in-out group-hover:scale-75" />
        <h3 className="text-lg font-semibold text-neutral-100">{name}</h3>
        <p className="max-w-lg text-sm text-neutral-400">{description}</p>
      </div>
      <div className="pointer-events-none flex w-full translate-y-0 flex-row items-center transition-all duration-300 group-hover:opacity-100 lg:hidden">
